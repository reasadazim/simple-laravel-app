        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-lg sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="mt-1 mb-4">
                        <x-primary-button>
                            <a href="{{ route('add_order') }}">{{ __('Add new order') }}</a>
                        </x-primary-button>
                    </div>
                    <div class="relative overflow-x-auto">
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 display" id="example">
                            ...
                            <tbody>
                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    <td class="px-6 py-4">
                                        {{ $order->customer->agent->agent_name }}
                                    </td>
                                    <td class="px-6 py-4">
                                        {{ date_format($order->created_at,"Y/m/d h:i:s a") }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <a href="{{ route('view_order', $order->id) }}" class="px-4 py-2 text-white bg-emerald-700 hover:bg-emerald-800 cursor-pointer rounded">View</a>
                                    </td>
                                    <td class="px-6 py-4">
                                        <a href="{{ route('edit_order', $order->id) }}" class="px-4 py-2 text-white bg-blue-700 hover:bg-blue-800 cursor-pointer rounded">Edit</a>
                                    </td>
                                    <td class="px-6 py-4">
                        ...
                        @else
                            No agent found. <a href="{{ route('add_order') }}" class="text-blue-600">Please add new order.</a>
                        @endif
                    </div>
                </div>
            </div>
